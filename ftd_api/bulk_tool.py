...
class BulkTool:
    ...
    def __init__(self, client,firepowerInst=None):
        """
        Parameters:
        ...
        self.client = client
        self.firepowerInst = firepowerInst
        ...
        if response.status_code == 200:
            ...
            while True:
                ...
                if status.status_code == 200 and status.json()['status'] not in ('IN_PROGRESS', 'QUEUED'):
                    ...
                    print('Error getting import job status: '+str(status.status_code)+" "+str(status))
        else:
            print('Triggering import failed with response code: '+str(response.status_code)+" "+str(response))
    ...
    def _do_upload_import_dict_list(self, 
                                    ...
                                    entity_filter_list=None,step_thru=False):
        """
        ...
        for item in dict_list:
            ...
            # File goes here
            ibody += json.dumps([item])+'\r\n'
            ibody += '\r\n--'+multipart_separator + '--\r\n'

            response = self.client.do_post_raw_with_base_url('/action/uploadconfigfile',ibody,additional_headers=additional_headers)
            response_json = response.json()
            ...
            if response.status_code == 200:
                ...
                if status_response['status'] == 'SUCCESS':
                    ...
                    uname, passwd,ip = self.firepowerInst.uname,self.firepowerInst.passwd,self.firepowerInst.ip
                    self.client = self.firepowerInst.connhandle.login_fpr_def(ip,uname,passwd)
                    #TODO: need put these in a seperate functionand need to deploy config on stop!!!! since we cant do use debuggingg in all instances and sometimes it error out with 'FAILED', 'statusMessage': 'Input data contains invalid objects.
                    response = self.client.do_post_raw_with_base_url('/action/uploadconfigfile', ibody, additional_headers=additional_headers)
                    response_json = response.json()
                    logging.debug(response_json)
                ...
                print('Error uploading import file response code: '+str(response.status_code)+" "+str(response))
                uname, passwd, ip = self.firepowerInst.uname, self.firepowerInst.passwd, self.firepowerInst.ip
                self.client = self.firepowerInst.connhandle.login_fpr_def(ip, uname, passwd)
