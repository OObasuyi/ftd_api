...
#TODO: need to intergrate new options for step by step config
class FPRType:
    """
    ...
    def bulk_configs(self,ip,pull_config:bool,file_path = getcwd(),pending:bool = False,sFDMendpoint=None,id_list = None,type_list = None,name_list= None,filter_local=False):
        self.ip = ip
        self.fprconnect = self.connhandle.login_fpr_def(self.ip,self.uname,self.passwd)
        bulk_client = BulkTool(self.fprconnect,firepowerInst=self)
        if pull_config:
            ...
            bulk_client.bulk_import(file_list, input_format='JSON', type_list=type_list, id_list=id_list, name_list=name_list, filter_local=filter_local)
...
if __name__ == "__main__":
    fpr = FPRType()
    ip = '192.168.1.2'
    fpr.bulk_configs(ip,pull_config=False,file_path = '/Users/osamuede/BumbleScrolls/Darth_Karman/full_config.txt',type_list=['internalcertificate','user','metadata','managementip','webuicertificate','datasslciphersetting'],filter_local = True)
